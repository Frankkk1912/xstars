...
## M0.4 实机验证清单（已执行 2026-08-31，结论见末尾）
...
- 结果/备注：✅ 2026-08-31 执行。宿主：WPS 365 教育高级版 12.1.0.28022 x64（同 M0.1–M0.3）；服务 3892、安装页 3890；加载项 1.2.2→1.2.3（版本递增修复同版本不重新解压问题）；实际 Origin：`file://`（M0.1–M0.3 实测，本轮一致）。显示缩放比例未记录（见步骤 6 限制）。
...
- 结果/备注：✅ `Application.InputBox(..., Type=8)` 在 Ribbon 回调上下文可用：框选返回正确地址/行列数/非空数；取消提示「用户取消（非错误）」，服务无错误请求。
...
- 结果/备注：✅ 两阶段三击流程按预期：标准品→样本→提交，服务收到两个矩形二维选区；再次点击重新开始。

### 3. 地址输入兜底

代码入口 `runM04AddressFallback()` 已存在，本轮只确认代码可被后续产品路径调用；不重复执行 Tkinter/地址交互实机验证。

- 结果/备注：✅ 代码入口存在并由 node 单测覆盖（19 pass），按计划未重复实机验证。
...
- 结果/备注：✅（含一项宿主发现）首次点击无输入框——发现 WPS 内嵌浏览器不支持 `window.prompt`（返回 null 被误判为取消）；已修复为宿主原生 `Application.InputBox(Type=2)`（commit `97820ce`，加载项 1.2.3）。修复后：选中图片时 `Selection` 不暴露 `Type` 属性，但 `ShapeRange` 可用，`CopyPicture(2, -4147)` 一次成功（未触发降级）。
...
| --- | --- | --- | --- |
| PNG | ✅ 854×480 | ✅ 2669×1500 | ✅ 5338×3000 |
| TIFF | ✅ 854×480 | ✅ 2669×1500 | ✅ 5338×3000 |
| JPG | ✅ 854×480 | ✅ 2669×1500 | ✅ 5338×3000 |
| PDF | ✅ 640.5×360pt | ✅ 640.56×360pt | ✅ 640.56×360pt |

12/12 组合全部成功；DPI 元数据（pHYs/IFD/MediaBox）经 Pillow 独立读取验证全部正确；像素全部等于剪贴板基线 854×480 × (dpi/96)；SHA-256 见 PR #1 证据评论。

### 6. 125% / 150% 显示缩放对比

分别在 125% 与 150% Windows 显示缩放下重复至少 PNG 300/600 DPI；核对像素尺寸、DPI 元数据和视觉清晰度是否随显示缩放产生非预期变化。

- 125% 结果/备注：✅（带限制）300/600 重复导出（12:16/12:17）与基线字节级一致（哈希相同），像素与元数据零漂移。
- 150% 结果/备注：同上字节级一致。限制：若 Windows 缩放切换未注销生效，本结论仅覆盖当前缩放环境；多缩放阻断矩阵归 M6.1。

### 7. 保存并重开

保存并完全退出 WPS，重开工作簿，确认源图片/Shape 仍存在且可再次导出；确认已生成的四格式文件仍可打开、大小与哈希未变化。

- 结果/备注：✅ 保存→完全退出→重开后再次导出成功且输出字节级一致；原 12 文件全部完好（SHA-256 未变）。
...
- 结果/备注：✅ COM `Ket.Application` 可用（版本 12.0），`GetActiveObject` 成功；同为用户级会话未提权，UAC 级别一致。
...
- 结果/备注：✅ 已判定（见结论）：剪贴板基线 854×480 = 192 有效 DPI；png@600 与 Lanczos(png@96→5338×3000) 平均差 0.123/255、99.89% 像素一致、拉普拉斯方差 405→37——高 DPI 输出为插值放大，无新增细节。用户补充对照：WPS 内置导出 320×180（原图原生 320×180），Excel XSTARS 导出 423×240（1.32× 插值）——两产品行为一致，细节天花板为源图分辨率。

### M0.4 结论（执行后填写）

- ELISA 最终交互路径：**InputBox(Type=8)** 在 Ribbon 回调上下文可用（与 Excel 体验一致），采用为主路径；两阶段交互可用，保留为兑底。
- Shape 导出最终路径（或 O3 fallback 决策需求）：**O3 已由用户拍板**——主目标 = XSTARS 生成图表，M5.4 采用 Python 重渲染（matplotlib 按目标 DPI 直出，不受源位图天花板限制，无需等 mac artifacts 方案）；任意选中图片 = bonus，剪贴板重编码路径已验证可用，插值放大可接受。无 fallback 需求。
- 阻断项：无。
- WPS/Windows/Python/Pillow 版本：WPS 365 教育高级版 12.1.0.28022 x64；Windows x64（具体版本号未记录）；Python 3.12（.venv）；Pillow 随项目依赖；显示缩放未记录（见步骤 6 限制）。
- 截图、导出文件与 SHA-256：17 个导出文件位于 `poc/wps/.gate0-artifacts/exports/`（12 基线 + 4 次重导出字节一致 + 1 个 350dpi 验证样例）；SHA-256 由编排器独立验证并记录于 PR #1。
- 执行日期与执行人：2026-08-31，用户执行；编排器独立验证文件指标。
