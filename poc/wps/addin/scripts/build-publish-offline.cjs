...
const serverUrl = process.env.XSTARS_WPS_PUBLISH_BASE || "http://127.0.0.1:3890/addin/"
...
function assertLoopbackUrl(value) {
    ...
    if (parsed.protocol !== "http:" || parsed.hostname !== "127.0.0.1" || !parsed.pathname.endsWith("/")) {
        throw new Error("XSTARS_WPS_PUBLISH_BASE must be an http://127.0.0.1 URL ending in '/'.")
    }
}
...
function readPublishList() {
    ...
    try {
        ...
        throw new Error(`The generated publish list is invalid JSON: ${error.message}`)
    }
}
...
async function main() {
    ...
    const wpsjsRoot = path.resolve(path.dirname(require.resolve("wpsjs/vite_plugins")), "..")
    const buildModule = require(path.join(wpsjsRoot, "src", "lib", "build.js"))
    const publishCli = path.join(wpsjsRoot, "src", "index.js")
    const publishListPath = path.join(wpsjsRoot, "src", "lib", "publishlist.json")
    ...
    try {
        ...
        run(process.execPath, [publishCli, "publish", "--serverUrl", serverUrl])
    } finally {
    ...
    if (publishList.length !== 1 || publishList[0].name !== project.name) {
        throw new Error(`Unexpected generated publish list: ${JSON.stringify(publishList)}`)
    }
    if (publishList[0].online !== "false" || publishList[0].url !== `${serverUrl}${project.name}.7z`) {
        throw new Error(`Unexpected offline publish record: ${JSON.stringify(publishList[0])}`)
    }

    fs.rmSync(deployRoot, { recursive: true, force: true })
    fs.mkdirSync(path.join(deployRoot, "addin"), { recursive: true })
    fs.copyFileSync(archivePath, path.join(deployRoot, "addin", path.basename(archivePath)))
    fs.copyFileSync(publishHtmlPath, path.join(deployRoot, "publish.html"))

    process.stdout.write(`Verified publish record: ${JSON.stringify(publishList[0])}\n`)
    process.stdout.write(`Staged offline installer: ${path.join(deployRoot, "publish.html")}\n`)
}
