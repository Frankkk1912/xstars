...
- [x] 2026-09-06 Review round 4（fresh-context reviewer）：P1 —— 导出对话框扩展名补全保留 Windows 侧行为变化（手输无后缀路径改为按所选格式命名保存），用户明确接受豁免（2026-09-06），跨平台行为由 `test_ensure_export_extension` 锁定；同轮实施 P2-1 Run 入口 darwin 守卫（残余 R1 缓解）、P2-2 导出对话框 topmost 降级、P2-3 导出 mainloop fd 2 静默、P2-4 README 平台声明与文档链接、P2-7 测试替身移除假 `book.path`、P2-8 `.gitignore` 补 `.review-tmp/`/`.ruff_cache/`；P2-5 `_read_json` 加固、P2-6 standard_curve 格式化拆分、P2-9 `elisa_fit_result` 还原处置记为 follow-up（PR 评论）。
