...
| rev 16 | 2026-09-18 | **xlwings 依赖漂移修复（路线 A）+ rev14/rev15 实现体事实修正**。① **事实修正**：rev14 声称「A–D 全闭合，CI 全绿」、rev15 宣布「reviewer OK」，但 A–D 三文件实现体（`build_pkg.py`/`uninstall.sh`/`.github/workflows/macos-support.yml`）实际从未提交，仅存在于工作区未暂存修改——已由 `d5e66c8` 补提交（2026-09-18）；r14/r15 中「CI 全绿」表述在 HEAD 上自 `d5e66c8` 起成立。② **xlwings 漂移**：xlwings 于 2026-09-16 发布 0.37.4；CI run 35314888948（@ d5e66c8）在 `--prepare-runtime` 阶段 `pip install <repo>` 解析到 0.37.4，而制品内嵌 bridge 为 0.37.0，`validate_xlwings_versions()` fail-closed → `macOS / Build installer package` FAIL。③ **路线 A 修复**：新增 `pinned_xlwings_requirement(assets_dir)` 纯函数——从 `XSTARS_mac.xlsm` 内嵌 bridge 版本（`embedded_xlwings_vba_version()`）派生精确 requirement，无第二版本源；`install_project()` 增加 `xlwings_requirement: str | None = None`参数；`assemble_staging()` 在调用前传入 pin；现有测试扩展 xlwings pin 位置与顺序断言；新增 `test_pinned_xlwings_requirement_derives_from_workbook`。④ **验证**：安装器单测 46 passed/1 skipped；全量 405 passed/14 skipped；ruff/compileall 干净；`pinned_xlwings_requirement()` 实测返回 `xlwings==0.37.0`。⑤ **残余**：完整运行时依赖锁（numpy/scipy/matplotlib 精确版本 ± hash）记入残余清单，本轮不实施（用户裁定：先恢复 CI 绿后再决策）。 | CI run 35314888948 失败日志实证；`d5e66c8` 提交记录；本轮 fix worker 报告 |
| rev 17 | 2026-09-18 | **xlwings 漂移第二漏斗：dev/test 环境钉版（路线 M）**。① **新漂移面**：rev16（c9bbc65）已钉构建侧运行时；但 `test_office_artifacts_embed_expected_normalized_vba_sources` 对照的是**测试环境**里已装的 `xlwings.bas`（来自 `pip install -e ".[dev]"`），而非构建运行时。CI run 35315689921（@ c9bbc65）实证：`macOS / Build installer package`（Python 3.12）装到 0.37.4，测试断言左 0.37.0 ≠ 右 0.37.4 → FAIL；两个 `Python 3.10` job 绿的原因是 PyPI 元数据 `xlwings 0.37.1+` 要求 `requires_python='>=3.11'`，3.10 运行时只能解析到 0.37.0——**属运气，不是契约**。② **路线 M 修复**（用户裁定）：新增 CLI 开关 `--print-xlwings-requirement`（互斥组成员，无副作用，调用 `pinned_xlwings_requirement()` 并 `print()`/`return 0`，不触发下载或 staging）；三个 job（`macos-tests`/`windows-tests`/`macos-pkg-build`）在「安装依赖」步骤后各插一步 `python -m pip install "$(python installer/mac/build_pkg.py --print-xlwings-requirement)"`（`shell: bash`）；`test_office_artifacts_embed_expected_normalized_vba_sources` 断言语义**不变**（仍是字节级溯源：制品内嵌 bridge == 上游同版本 wheel xlwings.bas，非循环：pin 提供版本号，测试验证字节）；新增 `test_print_xlwings_requirement_outputs_pin_and_has_no_side_effects`（monkeypatch pin 函数返回固定串 + command_runner 桩确认无副作用）。③ **验证**：`--print-xlwings-requirement` → `xlwings==0.37.0`；`--version` 回归 → `1.1.1`；安装器单测 47 passed/1 skipped；全量 406 passed/14 skipped；ruff/compileall/YAML 干净；workflow grep 恰 3 处。④ **CI 4/4 全绿（2026-09-18，run 35317417694 @ `0650381`）**：`macos-pkg-build` 真出包 `XSTARS-1.1.1.pkg`（artifact `macos-installer-pkg` 136MiB，retention 至 2026-09-25）+ 该 job 安装器测试 48 passed；macOS 3.10 → 406 passed/14 skipped；Windows 3.10 → 410 passed/10 skipped；VBA 门禁 pass。此前失败证据：run 35315689921（本 rev ② 所述）。 | CI run 35315689921 失败日志实证；本轮 fix worker 报告 |
...
**残余清单（可选改进，不在本轮实施）**：D11 归一化半径（`read_text` 连孤立 `\r` 一起吞；编码维度纯 ASCII 下非现存）；R24 零自动校验 + `*.sh` 不在 eol=lf；`Dictionary`/`ThisWorkbook` 模块无锚定（rev 6「三模块一致」声明 2/3 成立）；隐私扫描不覆盖压缩 `vbaProject.bin` 内部；postinstall awk 合并逻辑（生效实现）无行为测试、静态断言宽松项；build_pkg 多条 fail-closed 分支无测试（含 size 不匹配）；Pillow 无防漂移断言；D7 未拍板但被测试锁成双投契约；CI 可缓存 staging/downloads；**完整运行时依赖锁（精确版本 ± hash，覆盖 numpy/scipy/matplotlib/seaborn 等）**——rev14「最低 macOS 14.0」由浮动栈（numpy 2.5.3/scipy 1.18.1 wheel 标签 `macosx_14_0_arm64`）推出，上游再次发版即可能改写该承诺；rev16 路线 A 已治 xlwings，其余依赖仍浮动；本条待 T6.2 验收通过后由用户单独决策（B 路线）。**RK-01 处置**：staging 实测 Tcl/Tk 资源在位（`tcl9.0/init.tcl` 等）+ F1 冒烟 import ttkbootstrap 间接覆盖，sitecustomize 缓解推迟至 V-01 实测需要时。**验证终态（rev16）**：安装器单测 46 passed/1 skipped；全量 405 passed/14 skipped；CI Windows/macOS 3.10 绿；pkg 构建 job 待 rev16 push 后由 CI 真跑——真实 pkg 体积与三层 AppleDouble 零断言以该次构建结果为准（不沿用 rev13/rev14 的旧测量值）。
