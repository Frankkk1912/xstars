...
Download `XSTARS-1.2.pkg` from the project release and quit Excel before
...
2. From Terminal, install explicitly into the current user's home domain:
   ...
   ```bash
   sudo installer -pkg "/path/to/XSTARS-1.2.pkg" \
...
2. **Remove quarantine from this package:** inspect the path, then run:
   ...
   ```bash
   xattr -d com.apple.quarantine "/path/to/XSTARS-1.2.pkg"
   ```
