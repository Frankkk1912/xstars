...
' XSTARS Ribbon Callbacks — Installed (Shell + registry) mode
'
' ExePath() reads the install location from:
'   HKCU\Software\XSTARS\InstallPath
' Falls back to ThisWorkbook.Path if the registry key is missing.
'
...
Private Function ExePath() As String
    ...
    installDir = CreateObject("WScript.Shell").RegRead( _
        "HKCU\Software\XSTARS\InstallPath")
    ExePath = installDir & "\xstars\xstars.exe"
    Exit Function
fallback:
